...
- [IMPLEMENTATION.md](IMPLEMENTATION.md) — current version (v1.17 shipped; frozen Watch heat lamp persisted). Coverage map, next slice (below-BIOS + dense Low grid). Heat ladder / gap-fill locks live in [PATH_B_PHASE1_EXPERIMENT_PLAN.md](PATH_B_PHASE1_EXPERIMENT_PLAN.md). Do not skip ahead; update it when a slice finishes or App.md changes. New work is v1.1+, not a new P-phase.
