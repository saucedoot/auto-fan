...
- **Version:** v1.15 (product direction changed to Path B on 2026-09-21)
- **Status:** Watch measures three BIOS holds after the load freezes and gates fan-test claims by per-sensor stability and a minimum detectable effect. GPU generated policy still untrusted. The product is not validated until a BIOS → AUTO → BIOS comparison on the same locked heat. **Direction B supersedes the policy-only / not-a-curve-utility framing once App.md is updated.**
- **What the app actually does today (v1.15 behavior unchanged until a new code slice lands):** After Administrator, PawnIO, competing apps, and Find connected, Optimize opens an owned walk. Watch may raise GPU work per frame at 60 Hz until GPU Core is about 15 °C above idle, then freezes that workload. Still on BIOS/driver fans, it then settles and measures that same condition three times and labels CPU and GPU Core STABLE, DRIFTING, NOISY, or UNAVAILABLE. One bad sensor does not abort Watch. A fan-test Δ smaller than that sensor’s minimum detectable effect (`max(0.5 °C, range × 2)`) is None. A target that is not STABLE is Unknown. Individual fan tests that abort or cancel with a measured group skip pairs and may Continue to Hold (quieter conservative policy; confirmation still runs). Lost temps or a GPU reset retry Fans, not Hold. GPU-target influence stays Unknown, and NVIDIA GPU fans are not written for mapping, until Watch GPU Core rose about 15 °C. A fan-test GPU Δ is Unknown if duty did not rise by 5% or RPM did not rise. `Predict` adds a pair leftover only for exactly two fans; three or more use singles only. Unknown GPU influence contributes nothing. Confirmation will not publish a GPU guess below idle, or below start if GPU is already cooler than idle. What we learned shows the wander section, then the measured RPM versus settled °C graph in one card — only speeds we held until temperature stopped moving, not BIOS leftovers or one-sample hub flicker — not a BIOS curve, not `Predict()`, no live marker. Advanced stays a single column of optional retries and does not repeat that graph. Close, Stop, abort, GPU reset, lost temps, or crash returns motherboard fans to BIOS and NVIDIA fans to the driver. Latest live confirmation: GPU 39.0 °C vs 39.2 °C expected (no miss, no extra airflow).
...
- **What it does not do yet:** Curve editor UI on Home. Isolated GPU interleave (`R → speed → R`). Causal validity from power/clock. BIOS → AUTO → BIOS validation. CPU characterization when CPU is not STABLE. Case layout labels (P2). AMD GPU fan writes. Live Quiet–Cool slider changes while holding. Named-anchor Hold.
- **Next slice:** Documentation landed for Path B. Then propose the smallest code slice seeding editable Home curves from existing settled RPM/°C points — plan only in these docs, do not implement that slice in this PR. Isolated GPU only as needed for curve-quality data.
- **Blocked on:** Reviewed plan for the first curve-seed slice after these docs land. Do not loosen abort ceilings (CPU 90 / GPU 83).
...
| --- | --- | --- | --- |
| What one Optimize run does | Single characterization checklist: walk window; Watch 3× BIOS reference; settle, screen/refine, pairs only if individuals finished, one confirmation, anytime cancel, live headroom; aborted Watch / empty fan tests / lost temps / GPU reset retry; thermal abort with a partial map skips pairs and may Hold. BIOS A/B/A not built. | P9, P10, v1.5, v1.6, v1.9, v1.10, v1.15 | done (v1.15 wander gate; confirmation still vs Predict) |
...
| §2 Baseline | Observe without changing fans; idle / everyday / stronger (calibrate GPU work-per-frame at 60 Hz until ~+15 °C from idle, then freeze); three BIOS settle-holds; STABLE / DRIFTING / NOISY / UNAVAILABLE per CPU and GPU Core; MDE = max(0.5 °C, range × 2); record temps, RPM, duty, power, workload, clocks, rise/decay/settle; ambient when available. Settle seconds are a baseline metric, not fan-test dwell. No all-core High abort pass. Hidden DX12 3D GPU heat. | P4, v1.1, v1.2, v1.6, v1.8, v1.15 | done |
| §3 Individual fan tests | Screen then refine writable motherboard groups and one NVIDIA GPU fan set per card with tach; settle-by-measurement; stall exclusion; influence map; GPU-target Unknown and no NVIDIA GPU writes until Watch GPU Core rose ~15 °C; Δ below MDE is None; non-STABLE CPU/GPU targets are Unknown | P5, v1.5, v1.7, v1.9, v1.10, v1.15 | done |
| §4 Fan interactions | Pairs from measured same-target effects including slight; only after individuals finished; no GPU–GPU pair on the same card; one confirmation; no full factorial; skipped pairs are Unknown | P6, v1.7, v1.9, v1.10 | done (live: three pairs completed; pair leftovers only for two fans) |
...
| §6 Priorities | Quiet–Cool slider and optional targets; optional CPU/GPU stop-test fields that cannot exceed 90/83; RPM as noise proxy; live headroom toward cool end, not a louder default | P9, v1.2 | done |
| §7 Workload policies | Re-optimize the same model from CPU+GPU power; no second experiment tour per activity | P11 | done |
| §8 Diminishing returns | 1 °C plateau on a real multi-speed refinement curve; quietest plateau point; measured RPM vs °C graph | P8, v1.11, v1.12, v1.13, v1.14 | done (settled holds only; Penpot-matched card; no live marker; GPU policy untrusted) |
...
| §10 Explanations | Cooling-path writeup; wander section; confirmation line; RPM-as-noise Inferred; Low-load limits not Measured gaming truth; GPU cooling without ~15 °C rise, non-STABLE targets, and skipped pairs are Unknown | P10, v1.9, v1.10, v1.15 | done |

### Other App.md promises

| App.md | Need | Phase | Status |
| --- | --- | --- | --- |
| Product / Core Promise | Install, test this PC, produce a personalized strategy, user only chooses priorities; cancel still helps | P9, v1.5 | done |
| What the User Ultimately Gets | One **Optimize** action opens a walk; hide PID, hysteresis, airflow theory, experiment design; cancel still helps; Home is the path | P9, v1.3, v1.5 | done |
...
| What This Product Really Is, items 7–8 | Optimize temp/noise/responsiveness together; keep adapting by confirm-and-correct, not full re-test | P9, P11, P13 | done |
...
| Workload generation for baseline/experiments, only after P3, never without abort limits | P4+ (gated by P3), v1.8 | done. Calibrate GPU work per Present at 60 Hz, freeze for Low/fan tests. CPU workers stay Everyday. Device-loss and missing preferred temps abort. Rate-of-rise arms within 5 °C of the effective abort. Live Watch on this PC: GPU Core +15.1 °C from idle. |
| Apply the learned strategy as a live control loop (fan control is the actuator, not the product) | P9 | done |
| Distinguish Measured / Modeled / Inferred on every number shown to the user | P10 | done |
