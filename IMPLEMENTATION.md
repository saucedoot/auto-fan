...
- **What it does not do yet:** Below-BIOS duties. Dense Low/Hot duty grid. Everyday / Hot multi-heat mapping. Adaptive gap-fill. Confirmation-on-lamp. Hold abort when preferred temps go missing. Curve editor UI on Home. Isolated GPU interleave (`R → speed → R`). Causal validity from power/clock. BIOS → AUTO → BIOS validation. CPU characterization when CPU is not STABLE. Case layout labels (P2). AMD GPU fan writes. Live Quiet–Cool slider changes while holding. Named-anchor Hold.
- **Next slice:** Below-BIOS + absolute/dense duty grid on Low (Phase 1 plan slice 3 / D5). Do not start Everyday, Hot, gap-fill, confirmation-on-lamp, or Home curve editing in that slice. D6 Hold lost-temp abort remains a small safety PR, not the locked next experiment work.
- **Blocked on:** Pike review of the Phase 1 plan locks. Do not loosen abort ceilings (CPU 90 / GPU 83).
...
- **2026-09-21** — **Path B Phase 1 experiment locks** (plan only, no product code). Danny: heat ladder Idle (BIOS observe) → Everyday → Low → Hot (Hot = game-render-like, calibrate-then-freeze like Low, more GPU work-per-frame not all-core High, hard stop ~8 °C under abort; skip Hot and leave that band Unknown if it cannot sit ≥ 5 °C above Low). Below-BIOS absolute duties required (loud-first then quiet; restore every hold; stall/0 RPM skip). Low+Hot screen 15/30/45/60/75/90/100, refine 20/40/50/70/85 on movers; Everyday stays 20/40/70/100 + refine 55/85. Adaptive gap-fill in Phase 1: adjacent Measured duty gap ≥ 25 points → one midpoint (nearest 5%), max 2 per fan per heat; timeout/unsettled still not Measured. Build order after v1.17: below-BIOS dense Low → Everyday coarse → Hot if separates → gap-fill → point builder / confirm-on-lamp / BIOS→AUTO→BIOS → Phase 2 Home editor only after that. Phase 1 Hold stays two-end Quiet–Cool. ~25–40+ min first Optimize OK. Low thermal abort skips Everyday, pairs, and Hot. Next **code** slice is below-BIOS + dense Low (D5), not D6 and not the editor. Details: [PATH_B_PHASE1_EXPERIMENT_PLAN.md](PATH_B_PHASE1_EXPERIMENT_PLAN.md). Ceilings unchanged (CPU 90 / GPU 83).
