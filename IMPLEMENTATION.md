...
- **Version:** v1.15
- **Status:** Watch measures three BIOS holds after the load freezes and gates fan-test claims by per-sensor stability and a minimum detectable effect. GPU generated policy still untrusted. The product is not validated until a BIOS → AUTO → BIOS comparison on the same locked heat.
...
- **What it does not do yet:** Isolated GPU interleave (`R → speed → R`). Causal validity from power/clock. BIOS → AUTO → BIOS validation. CPU characterization when CPU is not STABLE. Case layout labels (P2). AMD GPU fan writes. Live Quiet–Cool slider changes while holding. Named-anchor Hold. Demand → duty graph.
- **Next slice:** Isolated GPU experiment (v1.16): GPU fans only when GPU is STABLE and Watch GPU rise was enough; interleave BIOS holds; invalidate a point if GPU power/clock is no longer the same experiment. Do not start A/B/A, CPU isolation, planner, extra heater, or GPU generated curves.
- **Blocked on:** a live Watch that reports GPU STABLE or an honest DRIFTING/NOISY/UNAVAILABLE. Do not loosen abort ceilings.
