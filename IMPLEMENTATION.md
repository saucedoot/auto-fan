...
- **Version:** v1.19 (Path B Phase 1 slice F1 — Everyday coarse probes)
- **Status:** After Low fan tests **Completed**, Optimize cools to the 60 °C gate and applies the **stored Everyday** Watch lamp (v1.17). Fans that moved a temperature at Low are probed with a coarse absolute grid (screen 20/40/70/100, refine 55/85; loud first, including quieter than BIOS). Restore after every hold. Samples store heat id **Everyday**. Missing Everyday lamp refuses that heat (no silent default). Low abort or cancel skips Everyday. Each heat has its own 30-minute cap. NVIDIA GPU writes at Everyday follow the +15 °C GPU-rise gate (Everyday usually skips them; GPU-target Everyday stays Unknown when the gate fails). GPU generated policy still untrusted. The product is not validated until a BIOS → AUTO → BIOS comparison on the same locked heat. **Path B (2026-09-21) supersedes the policy-only / not-a-curve-utility framing.** Phase 1 Hold stays today’s two-end Quiet–Cool policy. Experiment ladder locked Idle → Everyday → Low → Hot-if-separates ([PATH_B_PHASE1_EXPERIMENT_PLAN.md](PATH_B_PHASE1_EXPERIMENT_PLAN.md); Danny 2026-09-21).
- **What the app actually does today:** After Administrator, PawnIO, competing apps, and Find connected, Optimize opens an owned walk. Watch may raise GPU work per frame at 60 Hz until GPU Core is about 15 °C above idle, then freezes that workload and **stores Everyday + that frozen Low profile with the run**. Still on BIOS/driver fans, it then settles and measures that same condition three times and labels CPU and GPU Core STABLE, DRIFTING, NOISY, or UNAVAILABLE. One bad sensor does not abort Watch. Fan tests apply the **stored** Low profile, not a silent 4-pass default. Individual tests command the dense Low grid **below and above BIOS** (not “duties above BIOS only”). After Low **Completed**, they reuse the stored Everyday lamp on **Low-movers only** with the coarse grid (not Low’s dense list). `AlreadyAtMaxDuty` only if BIOS is already 100 and 100 is the only remaining point — a high BIOS speed does not skip quieter duties. A fan-test Δ smaller than that sensor’s minimum detectable effect (`max(0.5 °C, range × 2)`) is None. A target that is not STABLE is Unknown. A hold that hits the 90 s timeout while temperatures are still moving is stored as not settled; influence and pairs treat it as Unknown (`Temperatures were still moving at the end of the hold`), not a Measured Δ. Individual fan tests that abort or cancel with a measured group skip pairs **and Everyday** and may Continue to Hold (quieter conservative policy; confirmation still runs). Lost temps or a GPU reset retry Fans, not Hold. GPU-target influence stays Unknown, and NVIDIA GPU fans are not written for mapping, until Watch GPU Core rose about 15 °C. Everyday NVIDIA writes use Everyday GPU rise against that same 15 °C gate. A fan-test GPU Δ is Unknown if duty did not rise by 5% or RPM did not rise. `Predict` adds a pair leftover only for exactly two fans; three or more use singles only. Unknown GPU influence contributes nothing. Confirmation will not publish a GPU guess below idle, or below start if GPU is already cooler than idle. What we learned shows the wander section, then the measured RPM versus settled °C graph in one card — only speeds we held until temperature stopped moving, not BIOS leftovers, timeout holds, stalled 0 RPM, or one-sample hub flicker — not a BIOS curve, not `Predict()`, no live marker. Advanced stays a single column of optional retries and does not repeat that graph. Close, Stop, abort, GPU reset, lost temps, or crash returns motherboard fans to BIOS and NVIDIA fans to the driver. Latest live confirmation: GPU 39.0 °C vs 39.2 °C expected (no miss, no extra airflow).
...
- **What it does not do yet:** Hot lamp / Hot grid (F2). Point builder / confirm-on-lamp (F3). BIOS → AUTO → BIOS (F4). Adaptive duty gap-fill. Home curve editor. Isolated GPU interleave (`R → speed → R`). Causal validity from power/clock. CPU characterization when CPU is not STABLE. Case layout labels (P2). AMD GPU fan writes. Live Quiet–Cool slider changes while holding. Named-anchor Hold. D6 Hold abort on lost preferred temps.
- **Next:** F2 Hot-if-separates — only after Low Completed; calibrate extra GPU work from the stored Low lamp; skip and label Unknown unless preferred CPU or GPU sits ≥ 5 °C above Low. Do not start F3/F4, Home editor, gap-fill, or D6 in that slice.
- **Blocked on:** Pike review of this F1 PR. Do not loosen abort ceilings (CPU 90 / GPU 83).
...
| --- | --- | --- | --- |
| What one Optimize run does | Single characterization checklist: walk window; Watch 3× BIOS reference; persist Everyday + frozen Low heat; settle, absolute Low screen/refine (below and above BIOS), Everyday coarse on Low-movers only if Low Completed, pairs only if those finished, one confirmation, anytime cancel, live headroom; aborted Watch / empty fan tests / lost temps / GPU reset retry; thermal abort with a partial map skips pairs and Everyday and may Hold. Timeout holds are not Measured. Missing stored lamp refuses that heat (no silent DefaultLow / no silent Everyday). BIOS A/B/A not built. | P9, P10, v1.5, v1.6, v1.9, v1.10, v1.15, v1.16, v1.17, v1.18, v1.19 | done (v1.19 Everyday coarse) |
...
| §3 Individual fan tests | Screen then refine writable motherboard groups and one NVIDIA GPU fan set per card with tach; reuse the stored Watch Low lamp (refuse if missing); absolute Low duties 15/30/45/60/75/90/100 then refine 20/40/50/70/85 on movers (loud first, including below BIOS); after Low Completed, stored Everyday lamp + coarse 20/40/70/100 then 55/85 on Low-movers only; settle-by-measurement; 90 s timeout is not Measured; stall exclusion; influence map from Low; GPU-target Unknown and no NVIDIA GPU writes until Watch GPU Core rose ~15 °C (Everyday uses Everyday GPU rise for that gate); Δ below MDE is None; non-STABLE CPU/GPU targets are Unknown | P5, v1.5, v1.7, v1.9, v1.10, v1.15, v1.16, v1.17, v1.18, v1.19 | done (v1.19 Everyday coarse on Low-movers) |
...
| §7 Same curves, different heat | Live control may move along the Home curves as heat changes; no second experiment tour per activity. Phase 1 characterization ladder (Idle → Everyday → Low → Hot-if-separates): Everyday coarse probes shipped (v1.19); Hot not built. | P11, v1.19 | done (live heat still reuses the same model; curves not on Home yet; Hot fan probes not built) |
