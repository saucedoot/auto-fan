...
- **What it does not do yet:** Persist which heat lamp a run used. Below-BIOS duties. Everyday / multi-heat mapping. Confirmation-on-lamp. Curve editor UI on Home. Isolated GPU interleave (`R → speed → R`). Causal validity from power/clock. BIOS → AUTO → BIOS validation. CPU characterization when CPU is not STABLE. Case layout labels (P2). AMD GPU fan writes. Live Quiet–Cool slider changes while holding. Named-anchor Hold. PolicySession lost-temp handling beyond today’s restore.
- **Next slice:** Persist the Watch heat lamp on the run (Phase 1 slice 2 / D2). Do not start Home curve editing, Everyday heat, or below-BIOS duties in that slice.
...
| §3 Individual fan tests | Screen then refine writable motherboard groups and one NVIDIA GPU fan set per card with tach; settle-by-measurement; 90 s timeout is not Measured; stall exclusion; influence map; GPU-target Unknown and no NVIDIA GPU writes until Watch GPU Core rose ~15 °C; Δ below MDE is None; non-STABLE CPU/GPU targets are Unknown | P5, v1.5, v1.7, v1.9, v1.10, v1.15, v1.16 | done (v1.16 settled flag) |
