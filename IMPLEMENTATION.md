...
- **What it does not do yet:** Below-BIOS duties. Dense Low/Hot duty grid. Everyday / Hot multi-heat mapping. Confirmation-on-lamp. Hold abort when preferred temps go missing (D6 — Edges / Later, optional safety PR). Adaptive duty gap-fill (Later, not Phase 1). Curve editor UI on Home. Isolated GPU interleave (`R → speed → R`). Causal validity from power/clock. BIOS → AUTO → BIOS validation. CPU characterization when CPU is not STABLE. Case layout labels (P2). AMD GPU fan writes. Live Quiet–Cool slider changes while holding. Named-anchor Hold.
- **Next slice:** Below-BIOS + absolute/dense duty grid on Low (Phase 1 plan slice 3 / D5: screen 15/30/45/60/75/90/100, refine movers 20/40/50/70/85). Do not start Everyday, Hot, confirmation-on-lamp, gap-fill, or Home curve editing in that slice. D6 Hold lost-temp abort remains a small optional safety PR in Edges / Later — not the locked next experiment slice.
