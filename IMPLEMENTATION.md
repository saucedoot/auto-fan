...
- **What it does not do yet:** Everyday / Hot multi-heat mapping. Dense Hot grid. Confirmation-on-lamp. Hold abort when preferred temps go missing (D6 — Edges / Later, optional safety PR). Adaptive duty gap-fill (Later, not Phase 1). Curve editor UI on Home. Isolated GPU interleave (`R → speed → R`). Causal validity from power/clock. BIOS → AUTO → BIOS validation. CPU characterization when CPU is not STABLE. Case layout labels (P2). AMD GPU fan writes. Live Quiet–Cool slider changes while holding. Named-anchor Hold.
- **Next:** finish-Phase-1 plan (PR) — no code until Pike Ships the plan. Remaining required slices (see [PATH_B_PHASE1_FINISH_PLAN.md](PATH_B_PHASE1_FINISH_PLAN.md)): Everyday coarse → Hot-if-separates → point builder + confirm-on-lamp → BIOS → AUTO → BIOS. Do not start Home curve editing, gap-fill, or D6 in those slices. D6 Hold lost-temp abort remains a small optional safety PR in Edges / Later — not required to finish Phase 1.
- **Blocked on:** Pike Ship of the finish-Phase-1 plan. Do not loosen abort ceilings (CPU 90 / GPU 83).
...
| --- | --- | --- | --- |
| What one Optimize run does | Single characterization checklist: walk window; Watch 3× BIOS reference; persist Everyday + frozen Low heat; settle, absolute Low screen/refine (below and above BIOS), pairs only if individuals finished, one confirmation, anytime cancel, live headroom; aborted Watch / empty fan tests / lost temps / GPU reset retry; thermal abort with a partial map skips pairs and may Hold. Timeout holds are not Measured. Missing stored lamp refuses fan tests (no silent DefaultLow). BIOS A/B/A not built. | P9, P10, v1.5, v1.6, v1.9, v1.10, v1.15, v1.16, v1.17, v1.18 | done (v1.18 below-BIOS Low grid) |
...
| §3 Individual fan tests | Screen then refine writable motherboard groups and one NVIDIA GPU fan set per card with tach; reuse the stored Watch Low lamp (refuse if missing); absolute Low duties 15/30/45/60/75/90/100 then refine 20/40/50/70/85 on movers (loud first, including below BIOS); settle-by-measurement; 90 s timeout is not Measured; stall exclusion; influence map; GPU-target Unknown and no NVIDIA GPU writes until Watch GPU Core rose ~15 °C; Δ below MDE is None; non-STABLE CPU/GPU targets are Unknown | P5, v1.5, v1.7, v1.9, v1.10, v1.15, v1.16, v1.17, v1.18 | done (v1.18 absolute Low grid) |
...
| §7 Same curves, different heat | Live control may move along the Home curves as heat changes; no second experiment tour per activity. Phase 1 characterization ladder (Idle → Everyday → Low → Hot-if-separates) is locked in the experiment plan, not built yet. | P11 | done (live heat still reuses the same model; curves not on Home yet; Hot/Everyday fan probes not built) |
